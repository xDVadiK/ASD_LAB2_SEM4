...
private:
	...
	public:
		...
		T1 key; // Node Key
		T2 value; // Node Value
		T3 code; // The code of the symbol in the node
		...
		Element_Red_Black_Tree(int color = red, T1 key = 0, T2 value = 0, Element_Red_Black_Tree* parent_node = nullptr, T3 code = "", Element_Red_Black_Tree* left_node = nullptr, Element_Red_Black_Tree* right_node = nullptr)
		{
			this->color = color;
			this->key = key;
			this->value = value;
...
public:
	...
	void insert(T1 key, T2 value)
	{
		if (get_size_tree() != 0)
		{
			...
			while (!New_element_added)
			{
				...
				{
					current_element->right_node = new Element_Red_Black_Tree(red, key, value, current_element);
				...
				{
					current_element->left_node = new Element_Red_Black_Tree(red, key, value, current_element);
		...
		{
			root_node = new Element_Red_Black_Tree(black, key, value);
	...
	bool find(T1 key)
	{
		...
		else throw domain_error("The element cannot be found because the associative array is empty");
	};
	...
	void print()
	{
		if (get_size_tree() != 0)
		{
			...
			while (bft_iterator->has_next())
			{
				...
				cout << "symbol: " << Node->key << " | frequency occurrence: " << Node->value << " | code: " << Node->code << "\n";
			}
		}
		else throw domain_error("Red-black tree (associative array) is empty");
	}
